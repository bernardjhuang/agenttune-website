...
(function () {
  ...
  const INTEGRATIONS = [
    ...
    { id: "chatgpt-custom", name: "ChatGPT · custom instructions", sub: "Personal communication defaults.", badge: "Personal",
      steps: 'Open <strong>Settings → Personalization → Custom Instructions</strong> and add the block. Check the field’s current character allowance; trim the tuning or use a project or new chat if it is too long.' },
  ...
  window.renderIntegrations = function (tuning, containerOrOpts) {
    ...
    function updatePanel() {
      ...
      $("[data-status]").textContent = text ? text.length.toLocaleString() + " characters · copies the full prompt" : "Choose a tuning to create a prompt.";
    }
