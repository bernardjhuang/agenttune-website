...
test('all type pages place setup before raw content and keep secondary sections crawlable',()=>{
 for(const t of catalog.tunings){const file=new URL(t.page).pathname.slice(1)+'.html',html=enhance(read(file),file);assert.ok(html.indexOf('id="install"')<html.indexOf('id="editor"'),file);assert.match(html,/<article class="c-editor" id="editor">/);assert.doesNotMatch(html,/<details[^>]*id="editor"/);assert.ok(html.includes('id="integration-deep"'));assert.doesNotMatch(html,/gtag\("event"/);}
});
