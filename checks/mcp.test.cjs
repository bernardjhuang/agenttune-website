...
function conforms(schema,value){
 if(Array.isArray(schema.type)){assert.ok(schema.type.includes(value===null?'null':typeof value));return;}
...
test('MCP structured results match advertised schemas and retain readable text',async()=>{
 ...
 for(const [name,args]of [['list_tunings',{}],['get_tuning',{system:'mbti',slug:'entp'}],['get_test_spec',{test:'big-five'}],['get_test_spec',{test:'enneagram'}],['list_resources',{query:'Claude'}],['get_resource',{id:'claude-personality'}]]){
