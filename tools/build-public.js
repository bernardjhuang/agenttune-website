...
const ASSETS=['styles.css','data.js','integrations.js','compact-tunings.js','quiz-utils.js','consent.js','guide-prompts.js','resources/tools/core.js','resources/tools/app.js','resources/tools/tools.css','resources/tools/navigation.js','resources/tools/navigation.css','resources/enneagram/score.js'];
...
function build(out=path.join(ROOT,'dist')) {
  ...
  let v=0;
  (function stamp(dir){for(const entry of fs.readdirSync(dir,{withFileTypes:true})){const p=path.join(dir,entry.name);if(entry.isDirectory())stamp(p);else if(entry.name.endsWith('.html')){const html=fs.readFileSync(p,'utf8');const rel=path.relative(out,p).split(path.sep).join('/');const route='/'+rel.replace(/index\.html$/,'').replace(/\.html$/,'');const next=versionAssets(applyNavigation(html,route),hashes);if(next!==html){fs.writeFileSync(p,next);v++;}}}})(out);
