...
# (replace CLAUDE.md with the path that matches your surface — see table below)</code></pre>
    ...
    <footer class="lib-footer">
      <span>MIT · <a href="https://github.com/psyduckler/agenttune" target="_blank" rel="noopener">psyduckler/agenttune</a></span>
  ...
  <script>
    (function () {
      ...
      if (window.renderIntegrations) {
        window.renderIntegrations(TUNING, { only: ["claude-code", "claude-ai", "chatgpt-custom", "cursor", "gemini-gems", "codex-cli"] });
      }
...
function main() {
  ...
  console.log(`_redirects → ${redirectsPath} (${redirects.length} rules)`);
