...
<head>
  ...
  <style>
    ...
    .c-editor-body { display: grid; grid-template-columns: 56px minmax(0, 1fr); font-family: var(--font-mono); font-size: 13px; line-height: 1.75; }
    ...
    .c-code { padding: 22px 24px; white-space: pre-wrap; overflow-wrap: anywhere; min-width: 0; color: rgba(255,255,255,0.88); }
    ...
    @media (max-width: 720px) {
      .c-editor-body { grid-template-columns: 36px minmax(0, 1fr); }
...
<body class="lib-page">
  ...
  <main id="main" tabindex="-1">
    ...
    <article class="c-editor" id="editor">
      ...
      <div class="c-statusbar">
        ...
        <div class="c-statusbar-actions">
          ...
          <button class="c-status-btn is-strong" id="c-copy" type="button">⌘C · Copy base tuning</button>
        </div>
      </div>
    </article>
    ...
    <section class="lib-v2-jump" aria-label="Choose a model for your prompt">
      <div class="lib-v2-jump-eyebrow">Make it fit your model ↓</div>
      <div class="lib-v2-jump-grid">
        <button class="lib-v2-jump-btn" data-prompt-model="muse" type="button"><span class="lib-v2-jump-name">Muse</span><span class="lib-v2-jump-tag">Prompt</span></button>
        <button class="lib-v2-jump-btn" data-prompt-model="astra-6" type="button"><span class="lib-v2-jump-name">Astra 6</span><span class="lib-v2-jump-tag">Prompt</span></button>
        <button class="lib-v2-jump-btn" data-prompt-model="fable-5-1" type="button"><span class="lib-v2-jump-name">Fable 5.1</span><span class="lib-v2-jump-tag">Prompt</span></button>
        <button class="lib-v2-jump-btn" data-prompt-model="sol-6" type="button"><span class="lib-v2-jump-name">Sol 6</span><span class="lib-v2-jump-tag">Prompt</span></button>
        <button class="lib-v2-jump-btn" data-prompt-model="opus-5-5" type="button"><span class="lib-v2-jump-name">Opus 5.5</span><span class="lib-v2-jump-tag">Prompt</span></button>
        <button class="lib-v2-jump-btn" data-prompt-model="grok-4-7" type="button"><span class="lib-v2-jump-name">Grok 4.7</span><span class="lib-v2-jump-tag">Prompt</span></button>
      </div>
      <p class="lib-ctx-guides"><a href="#install">Other models, apps, and APIs →</a></p>
    </section>
...
<div id="integration-deep" data-integration-deep></div>
  ...
  <script>
    (function () {
      ...
      // Model shortcuts are handled by the shared prompt picker.
...
// ---------- Sitemap / headers / machine catalog ----------
...
// lastmod for a repo file: today if it has uncommitted changes (about to be
// committed), else its last commit date. Keeps sitemap dates honest without
// hand-editing.
function gitLastmod(relPath) {
  try {
    const opts = { cwd: ROOT, stdio: ["ignore", "pipe", "ignore"] };
    if (execSync(`git status --porcelain -- "${relPath}"`, opts).toString().trim()) return todayISO();
    return execSync(`git log -1 --format=%cs -- "${relPath}"`, opts).toString().trim() || todayISO();
  } catch {
    return todayISO();
  }
}
...
const STATIC_PAGES = [
  ...
  { route: "/tests/enneagram", file: "tests/enneagram.html", changefreq: "monthly", priority: "0.8" },
  ...
  { route: "/guides/muse-soul-md-templates", file: "guides/muse-soul-md-templates.html", changefreq: "monthly", priority: "0.8" },
  { route: "/guides/open-source-enneagram-test", file: "guides/open-source-enneagram-test.html", changefreq: "monthly", priority: "0.8" },
  ...
  { route: "/guides/what-is-meta-muse", file: "guides/what-is-meta-muse.html", changefreq: "weekly", priority: "0.8" },
  { route: "/guides/muse-vs-chatgpt-vs-claude-personality", file: "guides/muse-vs-chatgpt-vs-claude-personality.html", changefreq: "monthly", priority: "0.8" },
  { route: "/research/what-personality-type-is-muse", file: "research/what-personality-type-is-muse.html", changefreq: "monthly", priority: "0.8" },
...
const TEST_IDS = ["mbti", "enneagram", "disc", "attachment", "big-five"];

function writeSitemap(typePages) {
  const entry = (route, file, changefreq, priority, comment) =>
    (comment ? `\n  <!-- ${comment} -->\n` : "\n") +
...
function writeHeaders(typePages) {
  const L = [
    "# Auto-generated by tools/generate-library.js — do not edit by hand.",
    "# Cloudflare Pages header rules. Goal: agents fetch every .md freely;",
    "# search indexes consolidate on the canonical HTML pages.",
    "",
    "# Sitewide security headers (static responses; Functions set their own)",
    "/*",
    "  Strict-Transport-Security: max-age=31536000; includeSubDomains",
    "  X-Frame-Options: SAMEORIGIN",
    "",
    "# LLM index files — for agents to fetch, not for search result pages",
    "/llms.txt",
    "  X-Robots-Tag: noindex",
    "/llms-full.txt",
    "  X-Robots-Tag: noindex",
    "",
    "# Raw upstream mirror — fetchable, but keep out of indexes; the canonical",
    "# family is /library/{system}/{slug}(.md)",
    "/tunings/*",
    "  X-Robots-Tag: noindex",
    "",
    ...
    "  X-Robots-Tag: noindex",
    "",
    "# Resource downloads remain fetchable; discovery belongs to the guides",
    "/resources/*",
    "  X-Robots-Tag: noindex",
    "",
    "/guides/*.md",
  ...
  TEST_IDS.forEach((t) => {
    L.push(`/tests/${t}.md`);
    L.push(`  Link: <${SITE}/tests/${t}>; rel="canonical"`);
  });
  L.push("");
  L.push("# Tuning markdown mirrors → canonical = the library type page");
  typePages.forEach((p) => {
    L.push(`${p.route}.md`);
...
function writeCatalog(contacts) {
  ...
  const catalog = {
    name: "AgentTune tuning library",
    description:
      "Machine catalog of all AgentTune personality tuning files. Fetch a tuning's `md` URL (text/markdown, CORS-enabled) and install per its YAML front-matter. Full agent protocol: /llms.txt.",
    protocol_version: 1,
    generated: todayISO(),
    license: "MIT",
    site: SITE,
    upstream: "https://github.com/bernardjhuang/agenttune",
    docs: { llms: `${SITE}/llms.txt`, llms_full: `${SITE}/llms-full.txt`, hub: `${SITE}/library/` },
    tests: TEST_IDS.map((t) => ({ id: t, page: `${SITE}/tests/${t}`, spec_md: `${SITE}/tests/${t}.md` })),
    count: tunings.length,
    tunings
  };
  fs.writeFileSync(path.join(OUT_DIR, "index.json"), JSON.stringify(catalog, null, 2) + "\n");
  return tunings.length;
}

// llms.txt is still hand-maintained prose — fail loudly (not silently) when a
...
function checkLlms(typePages) {
  ...
  if (missing.length) {
    ...
    console.log("llms.txt: all library page URLs present ✓");
  }
}
...
function main() {
  ...
  contacts.forEach((c) => {
    ...
    typePages.push({ system: c.system, slug, route: `/library/${c.system}/${slug}` });
  ...
  const urlCount = writeSitemap(typePages);
  console.log(`sitemap.xml → ${urlCount} URLs (lastmod from git)`);

  const ruleCount = writeHeaders(typePages);
  console.log(`_headers → ${ruleCount} rule blocks (Pages limit: 100)`);

  const catCount = writeCatalog(contacts);
  console.log(`Machine catalog → library/index.json (${catCount} tunings)`);
