 ...
 *      GitHub. Parallels the /library/{system}/{slug}.md mirror.
...
- **Reverse-scored:** 18 items (marked ✓ below) — compute \`scored = 6 − raw\`
...
function buildLlmsBlock(allItems) {
  ...
  out += "Every test instrument's complete item list + scoring is also served as a same-domain Markdown mirror (parallel to the `/library/{system}/{slug}.md` tuning mirrors), so you can administer a test in a single fetch without rendering the page:\n\n";
